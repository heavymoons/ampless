...
import { extractRegistrableDomain, type DeployOptions } from './deploy-shared.js'
...
// `DeployOptions` + `extractRegistrableDomain` live in the dependency-free
// `deploy-shared` leaf so `preflight.ts` can use them without importing
// `deploy.ts` (which imports preflight back). Re-exported here so existing
// `./deploy.js` importers keep working.
export { extractRegistrableDomain } from './deploy-shared.js'
export type { DeployOptions } from './deploy-shared.js'
