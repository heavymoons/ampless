...
- **ページ上の MCP 接続情報** — `cms.config.ai.publicMcp` を有効にすると公開読み取り専用 MCP server を利用でき、接続情報は `/llms.txt` と管理画面 → MCP トークンに表示されます。読者向けリンク / QR コードは Phase D の discovery 作業まで後回しです。
