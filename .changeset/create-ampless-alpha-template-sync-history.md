...
re-publish with the latest template snapshot). 69 such
...
This entry replaces those 69 individual changesets on disk before the
beta dist-tag flip, so the eventual v1.0.0-stable CHANGELOG shows one
"alpha template-sync history" row instead of 69 noisy auto-sync rows.
