...
このトレードオフは意図的：ampless のターゲットはエンジニアがカスタマイズするサイトであり、プラグインのインストールはコード変更 + デプロイのステップです — エンジニアはインストール前に npm dep を審査し、CDK 再デプロイは許容範囲内。代替案（admin UI からランタイムで任意 JS をロード）はサンドボックス設計の難問になり、v2.0+ 探索に先送りされている。
