...
**admin preview iframe** もこの trust boundary 内に含まれます（v1 の明示的設計判断）。preview iframe は `sandbox="allow-scripts allow-same-origin"` で動作し、preview スクリプトに admin と同じ origin を与えます。これはサードパーティ embed widget に必要であり、preview content / plugin script を trusted とみなすという方針と整合しています（理由は同じ: エンジニア審査済みプラグイン、editor 著者の body）。完全なセキュリティ上のトレードオフと v2.0+ の代替案については下記の §Server-render fetch admin preview を参照してください。
...
4. `<PostForm>` は draft 変更を 250ms debounce して `fetch(previewEndpoint, { method: 'POST', body: JSON.stringify(draft) })` を呼び、レスポンス本文を `<iframe srcDoc>` (`sandbox="allow-scripts allow-same-origin"`、v1 trust boundary 拡張 — 下記参照) に流す。`<PostHistoryPanel>` も revision preview に同じパターンを使う; その場合、プレビュー対象の body は別の editor が作成した過去 revision の可能性があり（revision author ≠ preview viewer）、それも v1 では明示的に trust boundary 内と位置づける。

**Preview iframe sandbox — v1 trust boundary 拡張:** sandbox は `allow-scripts allow-same-origin`。`srcDoc` とともに使用すると iframe は embedding document の origin（= admin）を継承する。サードパーティ embed widget（YouTube SDK、x.com `widgets.js`）が必要とする — opaque-origin の（`allow-scripts` のみの）iframe では動作を拒否する。同時に same-origin により preview スクリプトは admin の auth state / 非 HttpOnly storage / DOM へのアクセスと authenticated same-origin XHR / fetch の発行が可能になる。v1 は **admin preview content と配線済 plugin script を全部 trusted とみなす** ことを明示的に設計判断として採用: エンジニアは npm install 前に plugin を審査するし、body content はこのサイトの trusted editor が作成する — カスタマイズベース CMS のポジショニングと整合（§Trust model (v1 scope)）。より安全な代替案（別 origin preview route + CSP / COEP / COOP）は v2.0+ でリアルなプラグインマーケットプレイスが必要になった場合に再検討する。
