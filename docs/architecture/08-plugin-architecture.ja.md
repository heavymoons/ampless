...
- **IAM**：Post と GSI に対する `dynamodb:Query` / `Scan`、KvStore に対する `dynamodb:Read`、PluginSecret に対する `dynamodb:GetItem` + `PutItem`（Phase 6a；暗号化キーの lazy 生成に write 権限が必要）、PostTag に対する `dynamodb:Write`、`public/plugins/*` に対する `s3:PutObject` / `DeleteObject`、加えて組み込みハンドラ用に `public/site-settings.json` への正確一致 grant。
- **ランタイムコンテキスト**：`listPublishedPosts()` は `byStatus` GSI に Query 1 発。`writePublicAsset(key, body, contentType)` は `public/plugins/{instanceId ?? name}/{key}` への書き込み。`ctx.secret<T>(key)` は PluginSecret table から ciphertext を読み取り AES-256-GCM 復号して plaintext を返す（invocation lifetime で plaintext をキャッシュ、Lambda lifetime で暗号化キーをキャッシュ、複合キーで cross-plugin 衝突防止）。
...
| admin 管理の secret settings | `PluginSecret` AppSync model（KvStore とは別テーブル）。`siteId` + `sk`（`plugins.<instanceId>.<fieldKey>`）で識別。admin/editor グループは `read`/`create`/`update`/`delete` 全権限あり — ただし `value` 列は **AES-256-GCM ciphertext**（base64）で保存されるため、AppSync 経由で読んでも暗号化済みブロブしか見えない。平文は DynamoDB に保存されない。防衛層多重化: AppSync/IAM 認証がアクセスを制御し、AES-256-GCM 暗号化が直接 DDB Console から覗いた場合の平文漏洩を防ぐ。サイト固有の 32-byte 暗号化キーは同テーブル内のブートストラップ行（`sk='__internal:encryption-key'`）に平文 base64 で保存される（ブートストラップ問題のため鍵自体は暗号化しない）。admin ブラウザ側は Web Crypto `crypto.subtle` (AES-GCM encrypt) で暗号化してから AppSync に書き込む。trusted Lambda 側は `node:crypto createDecipheriv('aes-256-gcm')` で復号する。`ctx.secret<T>(key)` は復号した **平文** を invocation lifetime でキャッシュし、暗号化キーは Lambda lifetime でキャッシュする。S3 mirror 経路には流れない。admin UI 書き込み: `setPluginSecret(field, instanceId, value)` / `clearPluginSecret` / `hasPluginSecret`。hook 側読み取り: `ctx.secret<T>(key)`。 | 実装済み (Phase 6a + Phase 6a follow-up)。`trust_level: 'trusted'` + `'secretSettings'` capability 必須。 |
