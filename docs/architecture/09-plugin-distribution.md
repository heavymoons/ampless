...
- `@ampless/plugin-ai-actions` — human-to-AI bridge links via `publicHtmlForPost` (Phase B of the AI-readable publishing roadmap). Injects a "View as Markdown" link (default on) to the post's `/<slug>.md` route, plus opt-in "Open in Claude" / "Open in ChatGPT" links that prefill a prompt with the absolute `.md` URL. The two external links default OFF — the `?q=` prefill is an unofficial community URL convention, not a documented contract from Anthropic or OpenAI. Requires `ai.markdownRoutes` to stay enabled; every link this plugin renders depends on the `.md` route. untrusted.
