...
- **IAM**: `dynamodb:Query` / `Scan` on Post + GSIs, `dynamodb:Read` on KvStore, `dynamodb:GetItem` + `PutItem` on PluginSecret (Phase 6a; write needed for lazy encryption-key creation), `dynamodb:Write` on PostTag, `s3:PutObject` / `DeleteObject` under `public/plugins/*`, plus an exact-match grant on `public/site-settings.json` for the built-in site-settings handler.
- **Runtime context**: `listPublishedPosts()` does one Query against the `byStatus` GSI; `writePublicAsset(key, body, contentType)` writes to `public/plugins/{instanceId ?? name}/{key}`; `ctx.secret<T>(key)` reads from the PluginSecret table, AES-256-GCM-decrypts the ciphertext using the per-site key, and caches the plaintext for the invocation lifetime (per-invocation cache, compound cache key prevents cross-plugin collisions).
...
| Admin-managed secret settings | `PluginSecret` AppSync model, separate table from KvStore. `siteId` + `sk` (`plugins.<instanceId>.<fieldKey>`) identifier. Admin/editor have full `read`/`create`/`update`/`delete` — but the `value` column stores **AES-256-GCM ciphertext** (base64), so reading via AppSync only exposes the encrypted blob. The plaintext never rests in DynamoDB. Defense in depth: AppSync authorization controls the write surface; AES-256-GCM encryption protects the plaintext even against direct DDB Console access. The per-site 32-byte encryption key lives in a special bootstrap row (`sk='__internal:encryption-key'`) in the same table; the key itself is unencrypted (bootstrap problem) but protected by the same AppSync/IAM authorization. The admin browser uses Web Crypto `crypto.subtle` (AES-GCM encrypt) to produce the ciphertext before calling AppSync. The trusted Lambda uses `node:crypto createDecipheriv('aes-256-gcm')` to decrypt. `ctx.secret<T>(key)` caches the decrypted **plaintext** for the invocation lifetime; the Lambda-lifetime cache holds the encryption key so it is fetched at most once per Lambda container. Never flows to the S3 site-settings mirror. Admin UI write path: `setPluginSecret(field, instanceId, value)` / `clearPluginSecret` / `hasPluginSecret`. Hook-side read: `ctx.secret<T>(key)`. | Implemented (Phase 6a + Phase 6a follow-up). Requires `trust_level: 'trusted'` + `'secretSettings'` capability. |
