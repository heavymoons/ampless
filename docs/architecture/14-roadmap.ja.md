...
- [x] Phase C — 公開読み取り専用 MCP: Next.js runtime が `/api/mcp` で提供する opt-in の匿名 JSON-RPC endpoint。4 つの read-only tool（`list_posts` / `get_post` / `search_posts` / `list_tags`）は published index query と `postToMarkdown` のみを使用。共有逐次 batch dispatch（≤50）、request / scan / body 上限、warm-instance circuit breaker、明示的に client-safe な validation / not-found message 用の `ToolUserError` を実装。`/llms.txt` と管理画面 → MCP トークンに endpoint 案内を追加。既存 admin MCP（Lambda Function URL + Bearer）は不変
- [x] Phase D — MCP discovery（2026-07-18 に rescope）: experimental な well-known discovery + 人間向け接続ガイド。`ai.mcpDiscovery`（デフォルト無効、`publicMcp` + http(s) の `site.url` が必須）は `/.well-known/mcp/catalog.json` の catalog（middleware で dot なしの内部 `/api/mcp/catalog.json` へ rewrite）と `/api/mcp/server-card` の Server Card を公開する。プロトタイプの `experimental-ext-server-card` 仕様（SEP-2127、まだ open / unmerged）に準拠し、vendored schema に対して AJV で検証済み。有効時は `/api/mcp` の `initialize` `serverInfo` が Card と一致する site 由来の reverse-DNS 名に切り替わる。接続ガイドと MCP Registry 公開手順は [docs/mcp.ja.md](../mcp.ja.md) に集約。**当初計画からの rescope:** `server.json` はコード生成しない（公式の `mcp-publisher init` フローが scaffold し、namespace 所有権を証明できるのは operator のみ — 代わりに手順をドキュメント化）。plugin-ai-actions のページ上リンク / QR affordance は取りやめ（well-known 経由の machine discovery + docs/admin surface でカバー）。サイト上の endpoint ヒント（`/llms.txt`、管理画面 → MCP トークン）は Phase B/C で ship 済み。
