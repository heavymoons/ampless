...
- [x] Phase 7 (embed plugin extension): `contentFields` capability (promoted from reserved) + `publicPostScript` capability + `Ampless.renderBody(post): Promise<ReactNode>` (alpha breaking) + `renderBodyHtmlString` for raw-route compat + admin editor extension installer (`@ampless/admin/editor`) + iframe-srcDoc preview pipeline (`'use server'` action threaded via page factory `renderPreviewAction` option). First plugins: `@ampless/plugin-youtube` + `@ampless/plugin-x-embed` (both `trusted`, served via `youtube-nocookie.com` and `platform.twitter.com/widgets.js`).
