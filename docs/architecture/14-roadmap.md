...
- [x] Phase A — canonical Markdown: server-safe `tiptapNodeToMarkdown` adapters on the `AmplessPlugin` manifest + `ampless.postToMarkdown(post)` in `@ampless/runtime` + public `/<slug>.md` route (middleware rewrite to internal `/md/<slug>`, published-only, unsupported nodes emit placeholders instead of silently dropping)
- [x] Phase B — AI index + on-page affordances: `/llms.txt` runtime route (site map with Markdown URLs, capped with explicit truncation note) + `@ampless/plugin-ai-actions` — a "View as Markdown" link (default on) plus opt-in "Open in Claude" / "Open in ChatGPT" links (`https://claude.ai/new?q=...` / `https://chatgpt.com/?q=...` prefill, default OFF — an unofficial community URL convention, not a documented contract) rendered after (or before) the post content via `publicHtmlForPost`. "Copy Markdown" (clipboard) was scoped out: the `publicHtmlForPost` sanitizer drops inline event handlers/`<button>`, and `publicPostScript` only supports external absolute script `src` (no inline-script channel) — revisit once an inline-script capability or plugin asset delivery exists. `/llms-full.txt` deferred until needed (event-driven S3 generation, rss/seo pattern)
- [x] Phase C — public read-only MCP: opt-in anonymous JSON-RPC endpoint served by the Next.js runtime at `/api/mcp`; four read-only tools (`list_posts` / `get_post` / `search_posts` / `list_tags`) use only published-index queries and `postToMarkdown`; shared sequential batch dispatch (≤50), request/scan/body limits, warm-instance circuit breaker, and `ToolUserError` for explicitly client-safe validation/not-found messages; endpoint guidance in `/llms.txt` and Admin → MCP tokens; admin MCP (Lambda Function URL + Bearer) unchanged
- [x] Phase D — MCP discovery (rescoped 2026-07-18): experimental well-known discovery + human connection guide. `ai.mcpDiscovery` (default off, requires `publicMcp` + http(s) `site.url`) publishes a catalog at `/.well-known/mcp/catalog.json` (middleware rewrite to the dot-free internal `/api/mcp/catalog.json`) and a Server Card at `/api/mcp/server-card`, following the prototype `experimental-ext-server-card` spec (SEP-2127, still open/unmerged) — AJV-validated against the vendored schema; when on, `/api/mcp` `initialize` `serverInfo` switches to a site-derived reverse-DNS name matching the Card. Connection guide + MCP Registry publishing steps live in [docs/mcp.md](../mcp.md). **Rescoped from the original plan:** `server.json` is not code-generated (the official `mcp-publisher init` flow scaffolds it and only the operator can prove namespace ownership — docs cover the flow instead); the plugin-ai-actions on-page link/QR affordance is dropped (machine discovery via well-known + the docs/admin surfaces cover it). On-site endpoint hints (`/llms.txt`, Admin → MCP tokens) shipped in Phase B/C.
