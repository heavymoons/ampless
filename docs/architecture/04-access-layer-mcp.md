...
- **Explicit client-safe errors.** Validation failures and expected not-found results throw `ToolUserError`, whose message the shared dispatcher may return to the client without logging. The class uses a `Symbol.for` brand so independently bundled `./jsonrpc` and `./public` entry points agree on the marker. All other exceptions — including an unbranded error merely named `ToolUserError` — are logged and masked by the public transport. A `ToolUserError` message must never include secrets, internal details, or unprocessed user input.
