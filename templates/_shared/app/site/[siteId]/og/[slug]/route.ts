...
function hasOgImage(p: unknown): p is AmplessPlugin & { ogImage: NonNullable<AmplessPlugin['ogImage']> } {
  return (
    typeof p === 'object' &&
    p !== null &&
    'ogImage' in p &&
...
export async function GET(_req: Request, { params }: Ctx): Promise<Response> {
  ...
  const post = await getPublishedPost(cleanSlug, { siteId })
  if (!post) return new Response('not found', { status: 404 })

  const plugin = (cmsConfig.plugins ?? []).find(hasOgImage)
